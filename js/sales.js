...
const salesHistoryTbody = document.getElementById("salesHistoryTbody");
...
const exportCsvSalesBtn = document.getElementById("exportCsvSalesBtn");
// Icon or button to close the modal
const closeAddSalesBtn = document.getElementById("close_add_sales_dialog");
// Cancel button within the add sale modal
const cancelAddSalesBtn = document.getElementById("cancel_add_sales_dialog");
...
Select_product.addEventListener("input", () => {
    ...
    const selectedProduct = InventoryData.find((item) => item.id === parseInt(Select_product.value));
    ...
    document.getElementById("product_quantity").innerText = `Available Quantity: ${selectedProduct.productQuantity}`;
...
add_sales_form.addEventListener("submit", (e) => {
    ...
    const selectedProduct = InventoryData.find((item) => item.id === parseInt(product));
    ...
    const sale = new Sale(id, product_Name, quantity, unitPrice, quantity * unitPrice, currentTimeStamp);
    SalesData.push(sale);
    ...
    const index_number = InventoryData.findIndex((item) => item.id === parseInt(selectedProduct));
